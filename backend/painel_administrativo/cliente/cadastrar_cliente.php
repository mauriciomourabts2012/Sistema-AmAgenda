...
try {
    ...
    /*
      FALLBACKS PARA EMPRESA NA SESSÃO
      Tenta buscar em vários pontos possíveis da sessão.
    */
    $idEmpresaSessao = 0;
    ...
    if ($nome === '') {
        $fields['cli_nome'] = 'Informe o nome do cliente.';
    } elseif (mb_strlen($nome) < 3) {
        $fields['cli_nome'] = 'O nome deve ter no mínimo 3 caracteres.';
    } elseif (mb_strlen($nome) > 140) {
        $fields['cli_nome'] = 'O nome deve ter no máximo 140 caracteres.';
    }

    if ($telefoneRaw === '') {
        $fields['cli_telefone'] = 'Informe o telefone (WhatsApp).';
    } elseif (strlen($telefone) < 10 || strlen($telefone) > 13) {
        $fields['cli_telefone'] = 'Telefone inválido. Informe DDD + número.';
    }

    if ($email !== null) {
        if (mb_strlen($email) > 160) {
            $fields['cli_email'] = 'O e-mail deve ter no máximo 160 caracteres.';
        } elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            $fields['cli_email'] = 'E-mail inválido.';
        }
    }

    if ($observacao !== null && mb_strlen($observacao) > 220) {
        $fields['cli_obs'] = 'A observação deve ter no máximo 220 caracteres.';
    }

    if (!in_array($status, ['ativo', 'bloqueado', 'inativo'], true)) {
        $fields['cli_status'] = 'Status inválido.';
    }
    ...
    /* ==========================================================
       VALIDAR TELEFONE NA EMPRESA
    ========================================================== */
    $stmt = $conexao->prepare("
        SELECT id_cliente, nome_completo, whatsapp_celular, status
        FROM cliente
        WHERE id_empresa = ?
    ...
    if (!$stmt) {
        throw new RuntimeException('Erro ao preparar validação do telefone do cliente: ' . $conexao->error);
    }

    $stmt->bind_param("is", $idEmpresaSessao, $telefone);

    if (!$stmt->execute()) {
        throw new RuntimeException('Erro ao executar validação do telefone do cliente: ' . $stmt->error);
    }

    $stmt->bind_result($clienteExistenteId, $clienteExistenteNome, $clienteExistenteTelefone, $clienteExistenteStatus);
    $clienteJaExiste = $stmt->fetch();
    $stmt->close();
    ...
    if ($clienteJaExiste) {
        out([
            ...
            'user_msg' => 'Já existe um cliente com este telefone nesta empresa.',
            'fields' => [
                'cli_telefone' => 'Telefone já cadastrado para esta empresa.'
            ]
        ], 409);
    }

    /* ==========================================================
       VALIDAR E-MAIL NA EMPRESA
    ========================================================== */
    if ($email !== null) {
        $stmt = $conexao->prepare("
            SELECT id_cliente
        ...
        if (!$stmt) {
            throw new RuntimeException('Erro ao preparar validação de e-mail do cliente: ' . $conexao->error);
        }

        $stmt->bind_param("is", $idEmpresaSessao, $email);

        if (!$stmt->execute()) {
            throw new RuntimeException('Erro ao executar validação de e-mail do cliente: ' . $stmt->error);
        }
        ...
        if ($stmt->num_rows > 0) {
            ...
            out([
                ...
                'user_msg' => 'Já existe um cliente com este e-mail nesta empresa.',
                'fields' => [
                    'cli_email' => 'E-mail já cadastrado para esta empresa.'
                ]
            ], 409);
        }
    ...
    if (!$stmt) {
        throw new RuntimeException('Erro ao preparar cadastro do cliente: ' . $conexao->error);
    }
    ...
    if (!$stmt->execute()) {
        ...
        throw new RuntimeException('Erro ao executar cadastro do cliente: ' . $error);
    }
    ...
    out([
        'ok' => true,
        'code' => 'CLIENT_CREATED',
        'user_msg' => 'Cliente cadastrado com sucesso.',
        'data' => [
    ...
    out([
        'ok' => false,
        'code' => 'INTERNAL_ERROR',
        'user_msg' => 'Erro interno ao cadastrar cliente.',
        'debug' => [
