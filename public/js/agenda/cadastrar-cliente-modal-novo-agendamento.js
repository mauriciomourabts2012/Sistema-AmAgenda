/* ==========================================================
   ...
   ✅ Após sucesso:
      ...
      - Fecha modal
      - Dispara evento
      - Recarrega a página mais rápido
      - Após recarregar abre o modalNovoAgendamento
========================================================== */
(() => {
  "use strict";
  ...
  /* ==========================================================
     TEMPO AJUSTADO
     Antes: 3500ms
     Agora: 1800ms (aprox. metade)
  ========================================================== */
  const SUCCESS_TOAST_TIME = 1800;
  const RELOAD_DELAY_AFTER_SUCCESS = 1900;

  const FORM_ID = "formNovoCliente";
  const MODAL_ID = "modalNovoCliente";
  const BTN_ID = "btnSalvarCliente";
  const MODAL_REABRIR_APOS_RELOAD = "modalNovoAgendamento";
  const STORAGE_KEY_MODAL_APOS_RELOAD = "abrirModalAposReload";
  ...
  function abrirModalPorId(modalId) {
    ...
    const btnAbrir = document.querySelector(`[data-abrir-modal="${modalId}"]`);
    if (btnAbrir) {
      btnAbrir.click();
      return true;
    }

    const elModal = document.getElementById(modalId);
    if (elModal) {
  ...
  function prepararAberturaModalAposReload(modalId) {
    try {
      sessionStorage.setItem(STORAGE_KEY_MODAL_APOS_RELOAD, modalId);
    } catch (err) {
      console.warn("[CadastrarCliente] Não foi possível salvar modal no sessionStorage.", err);
    }
  }

  function verificarModalPendenteAposReload() {
    try {
      const modalId = sessionStorage.getItem(STORAGE_KEY_MODAL_APOS_RELOAD);
      if (!modalId) return;

      sessionStorage.removeItem(STORAGE_KEY_MODAL_APOS_RELOAD);
      ...
      console.warn("[CadastrarCliente] Não foi possível recuperar modal pendente.", err);
    }
  }

  verificarModalPendenteAposReload();
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      document.dispatchEvent(new CustomEvent("cliente:cadastrado", {
        detail: json?.data || null
      }));
